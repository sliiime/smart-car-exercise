...
public class OrderServiceImpl implements OrderService {
  ...
  public List<OrderSuggestion> calculate(Order order) {
    ...
    for (OrderSuggestion suggestion : orderSuggestions) {
      int totalShippingCosts = 0;
      //the shipping cost for each shop is computed only once, regardless the number of items from the certain shop
      for(String shop : new HashSet<>(suggestion.getShopNames())) {
        totalShippingCosts += shippingCostPerShop.get(shop);
      }
      //add shipping costs to the total cost
      suggestion.setTotalCost(suggestion.getTotalCost() + totalShippingCosts);
    }
