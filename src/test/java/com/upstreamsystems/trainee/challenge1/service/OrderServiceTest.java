...
//  }
    ...
    @Test
    public void test_premium_order_calculated() throws Exception {
        CSVReaderTester csvReaderTester = new CSVReaderTester("/productstest2.csv", "/shopstest2.csv", "/citiestest1.csv");
        OrderService service = new OrderServiceImpl(csvReaderTester.getShops(),
                csvReaderTester.getProductPrices(), csvReaderTester.getCities());

        List<String> orderProducts = List.of("product1", "product2");


        List<OrderSuggestion> orderSuggestions = service.calculate(new Order("user1", orderProducts, true));


        int cnt = 0;
        int[] cost = {70, 90, 90, 110};
        for (OrderSuggestion suggestion : orderSuggestions) {
            assertEquals(cost[cnt], suggestion.getTotalCost());
            System.out.println(cnt++ + "=> " + suggestion);
        }
